...
CREATE TABLE `dw`.`fato_professor` (
  `id_fato_professor` bigint UNIQUE NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `dw`.`dim_curso` (
  `id_curso` bigint UNIQUE NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `dw`.`dim_disciplina` (
  `id_disciplina` bigint UNIQUE NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `dw`.`dim_departamento` (
  `id_departamento` bigint UNIQUE NOT NULL AUTO_INCREMENT,
...
CREATE UNIQUE INDEX ``dw`.dim_curso_index_0` ON `dw`.`dim_curso` (`de_curso`);

CREATE UNIQUE INDEX ``dw`.dim_disciplina_index_1` ON `dw`.`dim_disciplina` (`de_disciplina`);

CREATE UNIQUE INDEX ``dw`.dim_disciplina_index_2` ON `dw`.`dim_disciplina` (`id_pre_requisito_disciplina`);

CREATE UNIQUE INDEX ``dw`.dim_departamento_index_3` ON `dw`.`dim_departamento` (`de_departamento`);

CREATE UNIQUE INDEX ``dw`.dim_departamento_index_4` ON `dw`.`dim_departamento` (`de_campus_departamento`);

CREATE UNIQUE INDEX ``dw`.dim_departamento_index_5` ON `dw`.`dim_departamento` (`id_professor_coordenador`);
